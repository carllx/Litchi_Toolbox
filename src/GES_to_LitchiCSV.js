...
const toCSV = async(Radius,Shots)=>{
    // read the source to JSON
    const kfs = PObj['clipboard']['keyframes']

    // type:rotationY (0° - 180°)
    // Studio:Tilt/ Gimbal Pitch (30° - -90°)
    const Pitchs = kfs.filter(o => o.type === "rotationY")
        .map(o=> map_range(o.absoluteValue%360 ,0,180,30,-90))
    
    

    // type:rotationX (-∞° - ∞°)
    // Studio:Pan/ Heading (0° - 360°)
    const Headings = kfs.filter(o => o.type === "rotationX")
        .map(o => {
    ...
    // minValueRange: 1
    const Altitudes = kfs.filter(o => o.type === "altitude").map(o=>o.absoluteValue)



    // type:latitude
    const Lat = kfs.filter(o => o.type === "latitude").map(o=>o.absoluteValue)
    // type:longitude
    const Lon = kfs.filter(o => o.type === "longitude").map(o=>o.absoluteValue)
    // to csv 
    let CSV =[]
